...
class JiraHelper:
    ...
    def execute_jql(self, jql=None, **kwargs):
        ...
        try:
            ...
            if kwargs.get('verbose', True):
                ...
                _msg = ('\nDo not worry\nERROR was thrown, '
                        'Jira exception, here iss the message:\n'
                        f'{e}')
                config.logger.warning(_msg)
            jql_result = None
        return jql_result
    ...
    def get_transitions(self, issue=None):
        ...
        if issue is None:
            ...
            issues = self.execute_jql(f"project={self.project_slug} order BY created DESC")
            if not issues:
                config.logger.error(
                    "No issues returned for project slug %s", self.project_slug)
                return None
            issue = issues[0]

        if not isinstance(issue, Issue):
            config.logger.error("Expected an issue, instead received: %s", issue)
            return None
